...
======================================================
Gera:
  • Speed‏‑up, Eficiência, Overhead, ∆t, f (Amdahl) e α (Gustafson)
  • Gráficos em ./figs/
...
Ha = [0.000005, 0.000008, 0.000012, 0.000016, 0.000019, 0.000023, 0.000030, 0.000033, 0.000037, 0.000041]

langs = {"C Threads":Ct,"Julia":Ju,"OpenMP":OM,"Halide":Ha}
...
colors={"C Threads":"tab:orange","Julia":"tab:green","OpenMP":"tab:purple","Halide":"tab:red"}
...
line("Delta t","Dt ({})","Delta t (s)",log=True)
line("f (Amdahl)","f ({})","f")
line("Alpha (Gustafson)","a ({})","Alpha")
...
{res.round(4).to_string()}
    ...
    • Halide alcança Speed‑up médio {res.loc['Halide','S_med']:.1f}×; Eficiência {res.loc['Halide','E_med']:.2f}; Overhead {res.loc['Halide','O_med']:.0f} s.
    • Fração paralelizável média (f): Julia {res.loc['Julia','f_med']:.3g}, C Threads {res.loc['C Threads','f_med']:.3g}, OpenMP {res.loc['OpenMP','f_med']:.3g}, Halide {res.loc['Halide','f_med']:.3g}. Todas ≈1, logo o gargalo futuro é a memória. Alpha segue a mesma tendência.
